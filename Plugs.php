...
use service\AzureBlobStorageService;
...
class Plugs extends BasicAdmin
{
    ...
    public function upfile()
    {
        ...
        $path = $this->request->get('path', 'default');
        $this->assign('mimes', FileService::getFileMine($types));
        $this->assign('field', $this->request->get('field', 'file'));
        return $this->fetch('', ['mode' => $mode, 'types' => $types, 'uptype' => $uptype,'path'=>$path]);
    }
    ...
    public function upload()
    {
        ...
        $path = $this->request->post('path', 'default');
        $names = str_split($this->request->post('md5'), 16);

        $ext = strtolower(pathinfo($file->getInfo('name'), 4));
        $ext = $ext ?: 'tmp';
        $filename = $path.DIRECTORY_SEPARATOR.$names[0].DIRECTORY_SEPARATOR.$names[1].'.'.$ext;
        ...
        // 文件上传处理
        if(!is_dir("upload".DIRECTORY_SEPARATOR.$path)){
            mkdir("upload".DIRECTORY_SEPARATOR.$path,0777,true);
        }

        if (($info = $file->move("upload".DIRECTORY_SEPARATOR.$path.DIRECTORY_SEPARATOR.$names[0], $names[1].'.'.$ext, true))) {
    ...
    public function azure(){
        ...
        if (($file->move("upload".DIRECTORY_SEPARATOR.$path.DIRECTORY_SEPARATOR.$names[0], $names[1].'.'.$ext, true))) {

            if ((FileService::getFileUrl($filename, 'local'))) {
                //新增azure逻辑
                $fileFullPath = PUBLIC_URL.'/upload/'.$filename;
                $AzureBlobStorageService = new AzureBlobStorageService();
                $res = $AzureBlobStorageService->upload($fileFullPath,$filename);
                if(!empty($res)){
    ...
    public function upstate()
    {
        ...
        $filename =$path.DIRECTORY_SEPARATOR. join(DIRECTORY_SEPARATOR, str_split($post['md5'], 16)) . '.' . ($ext ?: 'tmp');
        $filename = str_replace("\\", "/", $filename);
        // 检查文件是否已上传
        ...
        // 需要上传文件，生成上传配置参数
        $data = ['uptype' => $post['uptype'], 'file_url' => $filename,'path'=>$post['path']];
        switch (strtolower($post['uptype'])) {
            ...
            case 'azure':
                $data['token'] = md5($filename . session_id());
                $data['server'] = FileService::getUploadAzureUrl();
                break;
//            case 'qiniu':
